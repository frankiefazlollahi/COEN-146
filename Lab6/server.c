...
* Date: 2/17/2022
* Title: Lab 6
* Description: UDP server with packets that have checksum, sequence number, acknowledgement number, and timer
*/
...
void serverSend(int sockfd, const struct sockaddr *address, socklen_t addrlen, int seqnum) {
	...
	if(rand() % 5 == 0) { // this gives a probability of 20% for a loss of ACK
		printf("Dropping ACK\n");
	}
	else {
		Packet packet;
