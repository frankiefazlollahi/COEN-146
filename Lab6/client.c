...
* Date: 2/17/2022
* Title: Lab 6
* Description: UDP Client with packets that have checksum, sequence number, acknowledgement number, and timer
*/
...
//client sending packet with checksum and sequence number and waits for acknowledgement, and sets up a time
		...
		// so there is an ACK to receive
			...
			recvfrom(sockfd, &recvpacket, sizeof(recvpacket), 0, (struct sockaddr *)address, &addrlen);
        		...
        		//check the sequence number
        			...
        			//if the ckechsum and sequence numbers are correct, break 
				//and return to the main to get the next packet to send
			} else {
				//good ACK, we're done
